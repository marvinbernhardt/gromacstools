...
def plot_data_header(data, header):
    ...
    if header['type'].lower() == 'xy':
        ...
        for column in range(len(data[0]) - 1):
            plt.plot(data[:, 0], data[:, column + 1],
                     label=header['legend'][column + 1])
...
def plot(filename):
    ...
    plot_data_header(data, header)


def load(filename):
    """parses a grace file and returns its data and header info.
    Does not work with all grace features!"""
    grace_types_legend = {
        'XY': ['x', 'y'],
        'XYDX': ['x', 'y', 'dx'],
        'XYDY': ['x', 'y', 'dy'],
        ...
        for line in f:
            if line.startswith('#'):
                ...
                if parts[0] == "@TYPE":
                    header['type'] = parts[1]
                    header['legend'] = \
                        grace_types_legend[header['type'].upper()]
                    ...
                    column = int(parts[1][1:]) + 1  # grace does not count x
