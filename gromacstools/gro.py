    ...
    with open(gro_file, "w") as f:
        ...
        for i, mol in enumerate(mols):
            for j, atomline in enumerate(mol):
                atomline = "{:>5}{}{:>5}{}".format(
                    i + 1, atomline[5:15], 3 * i + j + 1, atomline[20:]
                )
                f.write(atomline)
        for line in footer:
            f.write(line)
...
def set_atomname(gro_file, atomlist, atomname):
    ...
    if len(atomname) > 5:
        raise Exception(
            "atomname has {} characters, but can only have 5".format(len(atomname))
        )
...
def set_molname(gro_file, atomlist, molname):
    ...
    if len(molname) > 5:
        raise Exception(
            "molname has {} characters, but can only have 5".format(len(molname))
        )
    for i, line in enumerate(fileinput.input(gro_file, inplace=True)):
        if i - 2 in atomlist:
...
def translate_with_pbc(gro_file, vector, out_file="conf-shifted.gro"):
    """
    Tanslates the molecules in a gro_file along vector and uses pbc to put
    them all into the box.
    """

    x, y, z = vector[0], vector[1], vector[2]
    run_bash(
        f"gmx editconf -f {gro_file} -o conf-temp-trans.gro " f"-translate {x} {y} {z}"
    )
...
def generate_ffc_crystal_atomic(filename, n_atoms, box_length, mol_name, atom_name):
    ...
    if not n_atoms % 4 == 0:
        raise ValueError(
            "number of atoms not divisible by 4 (number of points in fcc unit " "cell)"
        )
    if not is_perfect_cube(n_atoms // 4):
        raise ValueError(
            f"number of atoms divided by 4 ({n_atoms // 4}) is not a cubic " "number"
        )
    ...
    with open(filename, "w") as f:
        ...
        f.write("argon\n")
        f.write("  {}\n".format(n_atoms))
        for x in np.linspace(0.0, box_length, num=nx, endpoint=False):
            for y in np.linspace(0.0, box_length, num=nx, endpoint=False):
                for z in np.linspace(0.0, box_length, num=nx, endpoint=False):
                    for lattice_point in lattice_points:
                        ...
                        f.write(
                            "{0: 5d}{1:>5s}{2:>5s}{0: 5d}"
                            "{3:8.3f}{4:8.3f}{5:8.3f}\n".format(
                                atom_nr, mol_name, atom_name, xpos, ypos, zpos
                            )
                        )
                        atom_nr += 1
...
def generate_ffc_crystal_water(filename, n_mols, box_length):
    ...
    if not n_mols % 4 == 0:
        raise ValueError(
            "number of molecules not divisible by 4 (number of points in fcc "
            "unit cell)"
        )
    if not is_perfect_cube(n_mols // 4):
        raise ValueError(
            f"number of molecules divided by 4 ({n_mols // 4}) is not a cubic " "number"
        )

    nx = round((n_mols // 4) ** (1 / 3))

    atoms = [
        ("OW", [0.0, 0.0, 0.00646032]),
        ("HW1", [0.08164904, 0.0, -0.05127557]),
        ("HW2", [-0.08164904, 0.0, -0.05127557]),
    ]
                    ...
                    for lattice_point in lattice_points:
                        for atom in atoms:
                            ...
                            f.write(
                                "{0: 5d}SOL    {1:3s}{2: 5d}"
                                "{3:8.3f}{4:8.3f}{5:8.3f}\n".format(
                                    mol_nr, atom_name, atom_nr, xpos, ypos, zpos
                                )
                            )
                            atom_nr += 1
                        mol_nr += 1
