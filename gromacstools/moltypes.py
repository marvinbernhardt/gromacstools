...
def generate_parameters_file(moltypes, nsamples, nblocks, nblocksteps, parameters_file="params.txt"):
    ...
    for h, moltype in enumerate(moltypes):
        for i, mol in enumerate(moltype['mols']):
            mols.append({'name': moltype['name'], 'mass': moltype['mass'], 'natoms': len(mol), 'moltypenr': h})
    ...
    with open(parameters_file, 'w') as f:
        ...
        fwrite(' '.join(map(str, [' '.join(map(str, moltype['abc_indicators'])) for moltype in moltypes])))
...
def shrink_box_remove_mols(moltypes, box_center, box_vectors, padding):
    ...
    padding space is added at the positive limits of the box for easier restarting of a simulation"""
    ...
    for moltype in newmoltypes:
        ...
        # remove those mols
        newmols = [mol for i, mol in enumerate(moltype['mols']) if not i in mol_index_out_of_box]
...
def write_gro_file(moltypes, out_filename, box):
    ...
    with open(out_filename, 'w') as f:
        ...
        f.write(str(len([atom for moltype in moltypes for mol in moltype['mols'] for atom in mol])) + '\n')
        ...
        for moltype in moltypes:
            for mol in moltype['mols']:
                for atom in mol:
                    string = "{:>5d}{:<5s}{:>5s}{:>5d}{:8.3f}{:8.3f}{:8.3f}{:8.4f}{:8.4f}{:8.4f}\n".format(molnr, moltype['name'], atom['name'], atomnr, atom['r'][0], atom['r'][1], atom['r'][2], atom['v'][0], atom['v'][1], atom['v'][2])
                    f.write(string)
                    atomnr += 1
                molnr += 1
